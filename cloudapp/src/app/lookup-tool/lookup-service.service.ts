...
export class LookUpService {
  ...
  handleRequest(item: any) {
    return this.searchPrimoApi(item).pipe(
      concatMap(results => {
        if (results.length > 0) {
          return of(results);  // If results found, return them
        } else {
          return this.getCourseData(item);  // Try fetching course-related data if no results found
        }
      }),
      catchError(error => {
  ...
  private getCourseData(row: any) {
    const courseURL = `/courses?q=name~${row['Course Semester']}%20AND%20course_number~${row['Course Number']}&format=json`;
